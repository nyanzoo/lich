...
impl OperatorConnection {
    pub(crate) fn connect(addr: String, our_port: u16, version: u128) -> Self {
        ...
        let read = std::thread::spawn(move || {
            let mut buffer = read_pool.acquire("backend decode", BufferOwner::OperatorFullDecode);
            ...
            let report = loop {
                let mut owned =
                    read_pool.acquire("backend decode", BufferOwner::OperatorFullDecode);
        ...
        let write = std::thread::spawn(move || {
            ...
            let mut owned = pool.acquire("backend join", BufferOwner::Join);
